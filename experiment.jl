...
md"""
Method: $(@bind validation PlutoUI.Select(["CV", "LOO"], default="CV"))
"""

# ╔═╡ 2f8d7d0e-2b2b-4429-ae36-c8cdf0c9455f
md"""
	Clustered: $(@bind clustered CheckBox())
	"""
...
models = [GaussianVariogram, ExponentialVariogram, SphericalVariogram]
...
let
	...
	for model in models
		...
		for range in ranges
			γ = model(range=range)
			n = string(nameof(model))
			t = n[begin:end-9] * " (" * n[begin] * ")"
			plot!(γ, 0, 60,
				  label="range = $range",
				  legend=:bottomright,
				  ylims=(0,1.1), title="$t")
	...
	plot(ps..., layout=(1,3), size=(900,200))
end
...
begin
	...
	for s in spacings
		...
		# sample drill holes
		dh = sample(deposit, BlockSampling(s))
		dc = dh |> Filter(s -> s.Z > quantile(dh.Z, 0.5))
		...
		# number of folds k
		kh = round(Int, nitems(dh) / 50)
		kc = round(Int, nitems(dc) / 50)

		# stores the result with the amount of samples for each setting
		push!(configs, (SPACING=s,
		                REGULAR_SAMPLES=nh,
		                CLUSTERED_SAMPLES=nc,
		                K_REGULAR=kh,
		                K_CLUSTERED=kc))
...
md"""
Please check the box $(@bind run PlutoUI.CheckBox()) if the setup is correct.
"""

# ╔═╡ 7f1ff788-938d-462a-b315-18d9ed42ee56
md"""
## Analysis of results
"""

# ╔═╡ 22514a6b-5559-4b30-b092-d3b584fe73c8
if run
	...
	@progress for real in 1:100
		...
		for s in spacings, m in models, r in ranges
				
			# sample drill holes
			dh = sample(deposit, BlockSampling(s))

			# filter samples above median value to
			# mimick clustered sampling campaigns
			if clustered
				dh = dh |> Filter(s -> s.Z > quantile(dh.Z, 0.5))
			end
			
	  		# reduce number of drill holes in case of LOO
			# to avoid extremely long experiment run times
			if validation == "LOO"
				dh = view(dh, 1:500)
			end
			
	  		# estimation problem for entire deposit
			problem = EstimationProblem(dh, domain(deposit), :Z)
		
	  		# Kriging with given variogram model and range
			solver = Kriging(
				:Z => (mean = 10, variogram = m(range = r), maxneighbors = 10)
			)
		
      		# compute k for k-fold validation such that each
			# fold has approximately 50 samples
			k = round(Int, nitems(dh) / 50)

			# use L1 loss for risk assessment
			ℒ = Dict(:Z => L1DistLoss())

			# choose validation method
			method = if validation == "CV"
				KFoldValidation(k, loss = ℒ)
			elseif validation == "LOO"  
				LeaveOneOut(loss = ℒ)
			end
		
	  		# cross-validatory error
			ϵ = error(solver, problem, method)[:Z]
		
	  		# store result of single experiment			 
			push!(results, (spacing = s, model = m, range = r, error = ϵ))

		end
	end

	# group experiments by spacing for further analysis
	groups = groupby(DataFrame(results), :spacing)
...
function errorplot(group)
	...
	df.cases = indexin(df.group, unique(df.group))

	s = df.spacing[1]
	
	@df df boxplot(:cases, :error, group = string.(:model),
...
# ╔═╡ 6b3f4c1c-732a-44ca-baf4-d3314aff4a91
# plotting the results in a boxplot chart
if run
	df = DataFrame(results)
	plots = [errorplot(group) for group in groups]
	plot(plots..., layout = (3,1), ylims = extrema(df.error) .+ (-0.1, +0.1))
end

# ╔═╡ 5e1e1c2c-01e0-48b0-8104-4948963b9fd1
if run
md"""
Reduction: $(@bind reduction PlutoUI.Select([median, mean]))
"""
end
...
function ranking(group)	
	...
	letter(x) = first(string(nameof(x)))
	error = DataFrames.transform(error, :model => ByRow(letter) => :model)
	error.model = string.(error.model) .* string.(Int.(error.range))
	sort(error, :error)
end;

# ╔═╡ a12ca180-92d9-4a41-be10-d4fe7c61bd32
# table with rankings of candidate variograms
if run
	ranks = [ranking(group) for group in groups]
	DataFrame([string(Int(rank.spacing[1])) => rank.model for rank in ranks])
end
...
git-tree-sha1 = "bf3e7fa17fad2213924394ef8340ddf9b0ee5d4f"
...
git-tree-sha1 = "f2c4971faf1389f3aa86610d13ef9537a2c04164"
...
git-tree-sha1 = "10bc2834a35034b7814b93b6b054beef795ffc0c"
...
git-tree-sha1 = "180330baaf5f48ff2b27efd2208dfe7998cbe3ff"
...
[[deps.GslibIO]]
deps = ["DelimitedFiles", "Meshes", "Printf", "Tables"]
git-tree-sha1 = "851ddf5c4bd35e398f8c104fb286fa96982d0c3e"
...
git-tree-sha1 = "fecfe63cd584b6e5a62614b21a8502fd0d3ff056"
...
git-tree-sha1 = "d5b2786238db55fd3fa840aaf6c16ebce9bf68d3"
...
git-tree-sha1 = "7f5c73dc667c24faf9bc9f286686e585ba1719c9"
...
git-tree-sha1 = "4618d126a5edc51ee638e411dffb5deb8db1cb81"
...
git-tree-sha1 = "004b5a44c923632bf0e29eced308d07a829a12a3"
